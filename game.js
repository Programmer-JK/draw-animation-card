...
function revealGenreCard(catIdx) {
  if (genreRevealState.usedCats.has(catIdx)) return;
  genreRevealState.usedCats.add(catIdx);

  const chip = document.querySelector(`.genre-chip[data-idx="${catIdx}"]`);
  if (chip) chip.classList.add('used');

  const deckEntry = catDecks[catIdx];
  const item = nextItem(deckEntry);
  const cardIdx = genreRevealState.usedCats.size - 1;
...
function draw() {
  ...
  if (currentMode === 'genre') {
    // 장르 모드: 카테고리 칩 클릭으로 한 장씩 공개
    genreRevealState = { usedCats: new Set() };
    document.getElementById('genreSelector').style.display = 'flex';
    document.querySelectorAll('.genre-chip').forEach(c => c.classList.remove('used'));
  } else {
    // 기본 모드: 6장 랜덤 공개
    document.getElementById('genreSelector').style.display = 'none';
